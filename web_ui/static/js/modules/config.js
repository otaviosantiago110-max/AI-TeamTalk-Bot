...
export async function fetchConfig() {
    try {
        ...
        showFlashMessage(t('web.flash.error_loading_config', { error: error.message }), 'danger');
    }
}
            ...
            if (inputType === 'checkbox') {
                inputHtml = `
                    <div class="form-check form-switch mb-3">
                        ...
                        <label class="form-check-label" for="${inputId}">${t(`web.config.field.${section}.${key}`)}</label>
                    </div>
                `;
            } else if (key === 'ai_system_instructions' || key === 'welcome_message_instructions') {
                 inputHtml = `
                    <div class="mb-3">
                        <label for="${inputId}" class="form-label">${t(`web.config.field.${section}.${key}`)}</label>
